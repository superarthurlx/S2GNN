...
def masked_nd(prediction: torch.Tensor, target: torch.Tensor, null_val: float = np.nan) -> torch.Tensor:
    """
    Calculate the Masked Normalized Deviation (ND) between the predicted and target values,
    ...
    Returns:
        torch.Tensor: A scalar tensor representing the masked normalized deviation.
    ...
    target = torch.nan_to_num(target)  # Replace any NaNs in the loss with zero

    return torch.sum(loss) / torch.sum(torch.abs(target))
