...
def standard_transform(data: np.array, dirs: dict, train_index: list, history_seq_len: int, future_seq_len: int, norm_each_channel: int = False) -> np.array:
    ...
    with open(dirs["output_dir"] + dirs["scaler"].format(history_seq_len, future_seq_len, norm_each_channel), "wb") as f:
...
@SCALER_REGISTRY.register()
def min_max_transform(data: np.array, dirs: str, train_index: list, history_seq_len: int, future_seq_len: int) -> np.array:
