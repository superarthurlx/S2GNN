...
from ..metrics import masked_mae, masked_mape, masked_rmse, masked_wape, masked_mse, masked_smape
...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: Dict):
        ...
        self.channel_norm = cfg.get("CHANNEL_NORM", True)

        cfg["scaler"] = "{0}/scaler_in_{1}_out_{2}_norm_each_channel_{3}.pkl"
        cfg["data"] = "{0}/data_in_{1}_out_{2}_norm_each_channel_{3}.pkl"
        cfg["index"] = "{0}/index_in_{1}_out_{2}_norm_each_channel_{3}.pkl"
        # setup graph
        self.need_setup_graph = cfg["MODEL"].get("SETUP_GRAPH", False)

        # read scaler for re-normalization
        self.scaler = load_pkl(cfg["scaler"].format(
            cfg["TRAIN"]["DATA"]["DIR"],
            cfg["DATASET_INPUT_LEN"],
            cfg["DATASET_OUTPUT_LEN"],
            cfg.get("CHANNEL_NORM", True)))
        # define loss
        self.loss = cfg["TRAIN"]["LOSS"]
        # define metric
        self.metrics = cfg.get("METRICS", {"MAE": masked_mae, "RMSE": masked_rmse, "MAPE": masked_mape, "WAPE": masked_wape, "SMAPE": masked_smape, "MSE": masked_mse})
        ...
        self.save_best_result = cfg.get("SAVE_RESULT", False)
    ...
    def build_train_dataset(self, cfg: Dict):
        ...
        """
        data_file_path = cfg["data"].format(
            cfg["TRAIN"]["DATA"]["DIR"],
            cfg["DATASET_INPUT_LEN"],
            cfg["DATASET_OUTPUT_LEN"],
            cfg.get("CHANNEL_NORM", True))
        index_file_path = cfg["index"].format(
            cfg["TRAIN"]["DATA"]["DIR"],
            cfg["DATASET_INPUT_LEN"],
            cfg["DATASET_OUTPUT_LEN"],
            cfg.get("CHANNEL_NORM", True))
    ...
    #     if self.adaptive_optim is not None:
    #         self.adaptive_optim.zero_grad()
    ...
    def test(self):
        ...
        # returns_all = self.rescale_data({"prediction": prediction, "target": target})
    ...
    @master_only
    def on_test_end(self, train_epoch: Optional[int], returns_all, greater_best: bool = False):
        """Callback at the end of testing.
        """
        pdb.set_trace()
        metric = self.meter_pool.get_avg("val_" + self.target_metrics)
        best_metric = self.best_metrics.get("val_" + self.target_metrics)

        if best_metric is None or (metric >= best_metric if greater_best else metric <= best_metric):
            # returns_all.pop('inputs')
            returns_all_keys = returns_all.keys()
            for key in returns_all_keys:
                returns_all[key] = returns_all[key].cpu()

            if self.save_best_result:
                np.savez(os.path.join(self.ckpt_save_dir, 'prediction_on_testset.npz'), **returns_all)
                log = "Prediction results saved"
                self.logger.info(log)
